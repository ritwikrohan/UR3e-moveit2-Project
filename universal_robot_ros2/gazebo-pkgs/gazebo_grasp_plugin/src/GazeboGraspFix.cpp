...
////////////////////////////////////////////////////////////////////////////////
GazeboGraspFix::GazeboGraspFix() { InitValues(); }

////////////////////////////////////////////////////////////////////////////////
GazeboGraspFix::GazeboGraspFix(physics::ModelPtr _model) { InitValues(); }
...
void GazeboGraspFix::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (!disableCollisionsOnAttachElem) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Using disable_collisions_on_attach "
  ...
  if (!updateRateElem) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Using update rate " << _rate);
  }
  ...
  if (!maxGripCountElem) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Using max_grip_count "
                           << this->maxGripCount);
  }
  ...
  if (!gripCountThresholdElem) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Using grip_count_threshold "
                           << this->gripCountThreshold);
  }
  ...
  if (!releaseToleranceElem) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Using release_tolerance "
  ...
  if (!armElem) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                        "GazeboGraspFix: Cannot load the GazeboGraspFix "
                        "without any <arm> declarations");
    return;
  }
  // add all arms:
  for (; armElem != NULL; armElem = armElem->GetNextElement("arm")) {
    ...
    if (!handLinkElem || !fingerLinkElem || !armNameElem) {
      RCLCPP_ERROR_STREAM(
          rclcpp::get_logger("grasp"),
          "ERROR: GazeboGraspFix: Cannot use a GazeboGraspFix arm because "
              << "not all of <arm_name>, <palm_link> and <gripper_link> "
                 "elements specified in URDF/SDF. Skipping.");
      continue;
    }
    ...
    if (!gripper.Init(_parent, armName, palmName, fingerLinkNames,
      ...
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                          "GazeboGraspFix: Could not initialize arm "
                              << armName << ". Skipping.");
    ...
    for (std::map<std::string, physics::CollisionPtr>::iterator collIt =
          ...
          this->collisions.end()) // this collision was already added before
      {
        RCLCPP_WARN_STREAM(
            rclcpp::get_logger("grasp"),
            "GazeboGraspFix: Adding Gazebo collision link element "
                << collName
  ...
  if (grippers.empty()) {
    RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("grasp"),
        "ERROR: GazeboGraspFix: Cannot use a GazeboGraspFix because "
  ...
  if (!this->contactSub) {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "Subscribing contact manager to topic " << topic);
    bool latching = false;
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                     "Advertising grasping events on topic grasp_events");
...
////////////////////////////////////////////////////////////////////////////////
bool GazeboGraspFix::ObjectAttachedToGripper(
    const std::string &gripperName, std::string &attachedToGripper) const {
  std::map<std::string, GazeboGraspGripper>::const_iterator gIt =
      grippers.find(gripperName);
  if (gIt == grippers.end()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                        "GazeboGraspFix: Inconsistency, gripper "
                            << gripperName
...
public:
  ...
  CollidingPoint(const CollidingPoint &o)
      : gripperName(o.gripperName), collLink(o.collLink), collObj(o.collObj),
        force(o.force), pos(o.pos), objPos(o.objPos), sum(o.sum) {}
...
bool CheckGrip(const std::vector<GzVector3> &forces, float minAngleDiff,
  ...
  if (((lengthRatio > 1) || (lengthRatio < 0)) &&
    ...
    RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("grasp"),
        "ERROR: CheckGrip: always specify a lengthRatio of [0..1]");
  ...
  for (it1 = forces.begin(); it1 != forces.end(); ++it1) {
    ...
    for (it2 = it1 + 1; it2 != forces.end(); ++it2) {
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                         "Angular distance is " << angle << " while the min is "
                                                << minAngleDiff / 2);
      // gzmsg<<"Angular distance between v1.len="<<v1.GetLength()<<" and
      // v2.len="<<v2.GetLength()<<": "<<angle*180/M_PI<<std::endl;
      if (angle > minAngleDiff / 2) {
        ...
        RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                           "Ratio is " << ratio << " while the min is "
                                       << lengthRatio);
...
void GazeboGraspFix::OnUpdate() {
  ...
  // first, copy all contact data into local struct. Don't do the complex grip
  // check (CheckGrip) within the mutex, because that slows down OnContact().
  ...
  // contPoints now contains CollidingPoint objects for each *object* and
  // *link*.

  // Iterate through all contact points to gather all summed forces
  // (and other useful information) for all the objects (so we have all forces
  // on one object).
  ...
  for (objIt = contPoints.begin(); objIt != contPoints.end(); ++objIt) {
    ...
    //  RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "Examining object
    //  collisions with "<<objName);
    ...
    for (lIt = objIt->second.begin(); lIt != objIt->second.end(); ++lIt) {
      ...
      // gzmsg << "Found collision with "<<linkName<<": "<<avgForce.x<<",
      // "<<avgForce.y<<", "<<avgForce.z<<" (avg over
      // "<<collP.sum<<")"<<std::endl;
      // RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "Found collision with
      // "<<linkName);
      objContInfo.appliedForces.push_back(avgForce);
      // insert the gripper (if it doesn't exist yet) and increase contact
      // counter
  ...
  // collect of all objects which are found to be "gripped" at the current
  // stage. if they are gripped, increase the grip counter. If the grip count
  // exceeds the threshold, attach the object to the gripper which has most
  ...
  for (std::map<std::string, ObjectContactInfo>::iterator ocIt =
    ...
    // gzmsg<<"Number applied forces on "<<objName<<":
    // "<<objContInfo.appliedForces.size()<<std::endl;
    // RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "Number applied forces on
    // "<<objName<<": "<<objContInfo.appliedForces.size());
    ...
    if (!CheckGrip(objContInfo.appliedForces, minAngleDiff, 0.10))
      continue;

    // add to "gripped objects"
    grippedObjects.insert(objName);

    // gzmsg<<"Grasp Held: "<<objName<<" grip count:
    // "<<this->gripCounts[objName]<<std::endl;
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "Grasp Held: " << objName << " grip count: "
                                      << this->gripCounts[objName]);
    ...
    // gzmsg<<"GRIPPING "<<objName<<", grip count "<<counts<<" (threshold
    // "<<this->gripCountThreshold<<")"<<std::endl;
    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GRIPPING " << objName << ", grip count " << counts
                                   << " (threshold " << this->gripCountThreshold
                                   << ")");

    // find out if any of the grippers involved in the grasp is already grasping
    // the object. If there is no such gripper, attach it to the gripper which
    // has most contact points.
    ...
    if (isAttachedToGripper) {
      // the object is already attached to a gripper, so it does not need to be
      // attached.
      RCLCPP_INFO_STREAM(
          rclcpp::get_logger("grasp"),
          "GazeboGraspFix has found that object that is already attached");
      // gzmsg << "GazeboGraspFix has found that object "<<
      //     gripper.attachedObject()<<" is already attached to gripper
      //     "<<gripperName; gripper.attachedObject()<<" is already attached to
      //     gripper "<<gripperName);
      continue;
    }
    ...
    if (gIt == grippers.end()) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                          "GazeboGraspFix: Inconsistency, gripper '"
                              << graspingGripperName
                              << "' not found in GazeboGraspFix grippers, so "
                                 "cannot do attachment of object "
                              << objName);
      continue;
    }
    ...
    if (graspingGripper.isObjectAttached()) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                          "GazeboGraspFix has found that object "
                              ...
                              << graspingGripperName << ", so can't grasp '"
                              << objName << "'!");
      continue;
    }

    RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"),
                       "GazeboGraspFix: Attaching " << objName << " to gripper "
                                                    << graspingGripperName
                                                    << ".");

    // Store the array of contact poses which played part in the grip, sorted by
    // colliding link. Filter out all link names of other grippers, otherwise if
    // the other gripper moves away, this is going to trigger the release
    // condition.
    // XXX this does not consider full support for an object being gripped by
    // two grippers (e.g. one left, one right).
    ...
    for (contPointsIt = contPointsTmp.begin();
      ...
      // gzmsg<<"Checking initial contact with "<<collidingLink<<" and
    ...
    if (!graspingGripper.HandleAttach(objName)) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                          "GazeboGraspFix: Could not attach object "
                              << objName << " to gripper "
                              << graspingGripperName);
    }
  ...
  for (gripCntIt = this->gripCounts.begin();
    ...
    if (grippedObjects.find(objName) != grippedObjects.end()) {
      // this object is one we just detected as "gripped", so no need to check
      // for releasing it... gzmsg<<"NOT considering "<<objName<<" for
      // detachment."<<std::endl;
      continue;
    }

    // the object does not satisfy "gripped" criteria, so potentially has to be
    // released.

    // gzmsg<<"NOT-GRIPPING "<<objName<<", grip count "<<gripCntIt->second<<"
    // (threshold "<<this->gripCountThreshold<<")"<<std::endl;
    // RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "NOT-GRIPPING
    // "<<objName<<", grip count "<<gripCntIt->second<<" (threshold
    // "<<this->gripCountThreshold<<")");
    ...
    // RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "Considering
    // "<<objName<<" for detachment.");

    // Object should potentially be detached now.
    // However, this happens too easily when just considering the count, as the
    // fingers in gazebo start wobbling as the arm moves around, and although
    // they are still close to the object, the grip is not detected any more. So
    // to be sure, we will check that the collision point (the place on the link
    // where the contact originally was detected) has not moved too far from
    // where it originally was, relative to the object.

    // get the initial set of CollidingPoints for this object
    // note that it is enough to use the initial contact points, because the
    // object won't have "slipped" after being attached, and the location of the
    // original contact point on the link itself is considered as a fixed point
    // on the link, regardless whether this point is currently still colliding
    // with the object or not. We only want to know whether this fixed point on
    // the link has increased in distance from the corresponding fixed point
    // (where the contact originally happened) on the object.
    ...
    if (initCollIt == this->attachGripContacts.end()) {
      RCLCPP_ERROR_STREAM(
          rclcpp::get_logger("grasp"),
          "ERROR: Consistency: Could not find attachGripContacts for "
              << objName);
    ...
    for (pointIt = initColls.begin(); pointIt != initColls.end(); ++pointIt) {
      ...
      // The same vector as \e oldObjDist, but calculated by the current world
      // pose of object and the current location of the initial contact location
      // on the link. This is the new distance from contact to object.
      GzVector3 newObjDist =
          currContactWorldPose - gazebo::GetPos(currObjWorldPose);

      // gzmsg<<"Obj Trans "<<cpInfo.collLink->GetName()<<": "<<relObjPos.x<<",
      // "<<relObjPos.y<<", "<<relObjPos.z<<std::endl; gzmsg<<"Cont Trans
      // "<<cpInfo.collLink->GetName()<<": "<<relContactPos.x<<",
      ...
      // gzmsg<<"Diff for link "<<cpInfo.collLink->GetName()<<":
      // "<<diff<<std::endl;
    ...
    if (cntRelease > 0) {
      // sufficient links did not meet the criteria to be close enough to the
      // object. First, get the grasping gripper:
      std::map<std::string, GazeboGraspGripper>::iterator gggIt =
          grippers.find(graspingGripperName);
      if (gggIt == grippers.end()) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("grasp"),
                            "GazeboGraspFix: Inconsistency: Gazebo gripper '"
                                << graspingGripperName
                                << "' not found when checking for detachment");
        continue;
      }
...
void GazeboGraspFix::OnContact(const ConstContactsPtr &_msg) {
  ...
  for (int i = 0; i < _msg->contact_size(); ++i) {
    physics::CollisionPtr collision1 =
        boost::dynamic_pointer_cast<physics::Collision>(gazebo::GetEntityByName(
            this->world, _msg->contact(i).collision1()));
    physics::CollisionPtr collision2 =
        boost::dynamic_pointer_cast<physics::Collision>(gazebo::GetEntityByName(
            this->world, _msg->contact(i).collision2()));
    ...
    if ((collision1 && !collision1->IsStatic()) &&
      ...
      //  RCLCPP_INFO_STREAM(rclcpp::get_logger("grasp"), "OBJ CONTACT!
      //  "<<name1<<" / "<<name2);
      ...
      // find out which part of the colliding entities is the object, *not* the
      // gripper, and copy all the forces applied to it into the vector 'force'.
      ...
      GzVector3 avgPos;
      // compute center point (average pose) of all the origin positions of the
      // forces appied
      for (int k = 0; k < contact.count; ++k)
      ...
      // To find out the collision point relative to the Link's local coordinate
      // system, first get the Poses as 4x4 matrices
      GzMatrix4 worldToLink = gazebo::GetMatrix(linkWorldPose);

      // We can assume that the contact has identity rotation because we don't
      // care about its orientation. We could always set another rotation here
      // too.
      ...
      // gzmsg<<"---------"<<std::endl;
      // gzmsg<<"CNT in loc:
      // "<<contactPosInLocal.x<<","<<contactPosInLocal.y<<","<<contactPosInLocal.z<<std::endl;

      /*GzVector3 sDiff=avgPos-linkWorldPose.pos;
      gzmsg<<"SIMPLE trans: "<<sDiff.x<<","<<sDiff.y<<","<<sDiff.z<<std::endl;
      gzmsg<<"coll world pose: "<<linkWorldPose.pos.x<<",
      "<<linkWorldPose.pos.y<<", "<<linkWorldPose.pos.z<<std::endl;
      gzmsg<<"contact avg pose: "<<avgPos.x<<", "<<avgPos.y<<",
      "<<avgPos.z<<std::endl;
      ...
      // gzmsg<<"Average force of contact= "<<avgForce.x<<", "<<avgForce.y<<",
      // "<<avgForce.z<<" out of "<<force.size()<<" vectors."<<std::endl;
    }
  }
}
